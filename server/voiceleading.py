...
parser.add_argument("key", help="A valid key")
parser.add_argument("tag", help="A tag to identify the user")
...
def create_midi_from_progression(progression):
	...
	for index, chord in enumerate(progression):
		...
		if index == len(progression) - 2:
			duration = 2
	binfile = open("server/data/" + args.tag + ".mid", 'wb')
...
def run(is_major=True):
	...
	### Assign a key based on the args
	key = args.key
